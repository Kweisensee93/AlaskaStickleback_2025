...
### We only need the script once, change with an if check so its only built once
if [ ! -f "/storage/homefs/kw23y068/software/scripts/annotate_sv.R" ]; then

cat > /storage/homefs/kw23y068/software/scripts/annotate_sv.R << 'EOF'
#!/usr/bin/env Rscript
...
Rscript /storage/homefs/kw23y068/software/scripts/annotate_sv.R \
    ${VCF_OUT} \
