#!/bin/bash
#SBATCH --job-name=Delly_BCFPostProcessing
#SBATCH --output=/storage/homefs/kw23y068/logfiles/Delly_BCFPostProcessing_%j.out
#SBATCH --error=/storage/homefs/kw23y068/logfiles/Delly_BCFPostProcessing_%j.err
...
MERGE_DIR="${OUTPUT_DIR}/Delly_Merge_BCFtools_${START_PAD}_${END_PAD}"

#INPUT_VCF=SPI_BJO_ROS_sites.vcf.gz # we will try not to modify this one
INPUT_VCF=${MERGE_DIR}/Delly_BCFmerged_${START_PAD}_${END_PAD}.vcf.gz
VCF_FOLDER=${MERGE_DIR}/TMP_VCF
OUTPUT_VCF=${MERGE_DIR}/Delly_BCFmerged_filtered_${START_PAD}_${END_PAD}.vcf
...
mkdir -p ${VCF_FOLDER}
cp -f $INPUT_VCF ${VCF_FOLDER}/raw.vcf.gz
gunzip -f ${VCF_FOLDER}/raw.vcf.gz
...
#     ${VCF_FOLDER}/raw.INS.vcf \
#     > ${VCF_FOLDER}/raw.INS.info1
...
# With the original setup, we drop all sample columns, which hinders downstream merging
# Replace ALT field with CONSENSUS (preserves all other columns)
awk 'BEGIN{OFS="\t"} 
    /^#/ {print; next} 
    {
        match($8, /CONSENSUS=([^;]+)/, arr);
        if (arr[1] != "") $5 = arr[1];
        print
    }' ${VCF_FOLDER}/raw.INS.vcf > ${VCF_FOLDER}/raw.INS.fixed.vcf

# Merge everything back
echo "Merging INS back with other SV types..."
grep "^#" ${VCF_FOLDER}/raw.vcf > ${VCF_FOLDER}/raw_sorted.noTRA.vcf
(grep -v "^#" ${VCF_FOLDER}/raw.noTRA.vcf; grep -v "^#" ${VCF_FOLDER}/raw.INS.fixed.vcf) | \
    sort -k1,1 -k2,2n >> ${VCF_FOLDER}/raw_sorted.noTRA.vcf
...
bcftools query -f '%CHROM\t%POS\t%INFO/END\n' \
    ...
    > ${VCF_FOLDER}/raw_sorted.noTRA.info
